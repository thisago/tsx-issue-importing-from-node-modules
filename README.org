#+title: tsx-issue-importing-from-node-modules
#+auto_tangle: t

When any of the parent directories are named =node_modules=, ~tsx~ doesn't loads =tsconfig.json=.
...
Defining the =node_modules/my-lib/tsconfig.json=. In this case, configuring the =@/= path as example.
...
Defining a dummy module (=node_modules/my-lib/add.ts=) to be imported by entry-point:
#+begin_src typescript :tangle node_modules/my-lib/add.ts :exports both :eval never
export function add(a, b) { return a + b; }
#+end_src

Entrypoint, which relies on tsconfig, =node_modules/my-lib/index.ts=:
...
Require stack:
- /home/user/Documents/repos/tsx-issue-importing-from-node-modules/node_modules/my-lib/index.ts
